    ...
    <div class="el-right">
      ...
      >
        <el-form-item class="chapter_name" label="章节名称：" prop="title">
          <el-input v-model="ruleForm.title" placeholder="示例，第一章 猛虎下山"></el-input>
        </el-form-item>
        ...
        <el-form-item label="章节说：" :rows="3" prop="zuozhe">
          <el-input type="textarea" v-model="ruleForm.chapterSays" placeholder="请输入内容"></el-input>
        </el-form-item>
        <el-form-item>
          <el-button type="primary" size="small" @click="modifyArticle">修改草稿</el-button>
          <el-button size="small" @click="deletedraft">删除草稿</el-button>
          <el-button type="primary" size="small" @click="saveForm">立即发布</el-button>
        </el-form-item>
      </el-form>
      <el-dialog title="提示" :visible.sync="dialogVisible" width="30%" :before-close="handleClose">
        <div class="el-icon-warning"></div>
        <span style="margin-left:10px">删除之后将无法恢复，确定删除草稿?</span>
...
export default {
  ...
  methods: {
    ...
    async handlelist() {
      ...
      this.list = [];
      if (this.booksdata.booksdraft.length !== 0) {
        this.list = this.booksdata.booksdraft.reverse();
    ...
    async saveForm() {
      ...
      this.ruleForm.day =
        year + "-" + month + "-" + day + " " + hh + ":" + mm + ":" + ss;
           var desc=this.ruleForm.content.replace(/\s+/g,"");   
          this.ruleForm.sizelength = desc.length;
      let data = await draftInsertApi(
        this.ruleForm,
        this.booksdata._id,
        this.booksdata.booksName
      );
    ...
    async deleteArticle() {
      ...
      let data = await deleteArticle2(this.booksdata._id, this.oldday);
      if (data.data.status == 1) {
        this.$message({
          message: data.data.info,
          type: "success"
